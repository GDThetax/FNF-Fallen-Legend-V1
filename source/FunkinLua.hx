...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "cameraPulse", function(camera:String, duration:Float) {
			cameraFromString(camera).fill(FlxColor.BLACK);
			cameraFromString(camera).fade(FlxColor.TRANSPARENT, duration);
		});
		...
		Lua_helper.add_callback(lua, "startDialogue", function(dialogueFile:String, music:String = null) {
			...
			if(FileSystem.exists(path)) {
				var shit:Array<String> = sys.io.File.getContent(path).split("\n");
				if(shit.length > 0) {
