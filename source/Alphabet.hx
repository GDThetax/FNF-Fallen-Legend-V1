...
class Alphabet extends FlxSpriteGroup
{
	...
	public var blackVer:Bool;

	public var typingSpeed:Float = 0.05;
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, typed:Bool = false, ?typingSpeed:Float = 0.05, ?textSize:Float = 1, ?blackVer:Bool = false)
	{
		super(x, y);
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				var letter:AlphaCharacter = new AlphaCharacter(xPos, 0, textSize, blackVer);
	...
	public function startTypedText(speed:Float):Void
	{
		...
		if(speed <= 0) {
			...
			dialogueSound = FlxG.sound.play(soundDialog, 0.01);
		} else {
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:AlphaCharacter = new AlphaCharacter(30 * loopNum, 0, textSize);
				var letter:AlphaCharacter = new AlphaCharacter(xPos, 55 * yMulti, textSize, blackVer);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
...
class AlphaCharacter extends FlxSprite
{
	...
	public function new(x:Float, y:Float, textSize:Float, ?black:Bool = false)
	{
		super(x, y);
		var tex = Paths.getSparrowAtlas(black ? 'alphabetBlack' : 'alphabet');
		frames = tex;
