...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['[COM] Fade Screen Black', "Value 1: fade time\nValue 2: hold time"],
		['[COM] Flash Screen', "Value 1: flash duration"],
		['[COM] Tween camera zoom', "Value 1: zoom value\nValue 2: zoom time"],
		['[DIS] camera zoom', "Value 1: zoom duration\nValue 2: zoom amount"],
		['[DIS] hanging Flash', "flash hanging body"],
		['[DIS][ANA] Toggle Rain Effect', "Value 1: Toggle state\nValue 2: Variance (1 is heavy, 2 is light)"],
		['[ANA] Toggle Anamnesis Words', "Value 1: Duration\nValue 2: just black screen (0 false, 1 true)"],
		['[ANA][ER] Show Memory Message', "Value 1: Visibility Duration"],
		['[ER] toggle side bars', 'Value 1: Toggle'],
		['[ER] set Sophie opacity', "Value 1: Opacity\nValue 2: fade time"],
		['[ER] toggle vertical bars', 'Value 1: Toggle'],
		['[ER] Toggle drop zoom', 'Value 1: toggle state'],
		['[TL]  Heart Effect', 'Value 1: Toggle'],
		['[ANA][ER] memory warning', 'pretty self explanatory'], 
		['[TRA] Claw Health Drain', 'Value 1: Drain duration'],
		['[TRA] saw strum removal distraction', 'Value 1: Distraction time\nValue 2: Strum ID (3R, 2G, 1B, 0P)'],
		['[TRA] Stage zoom out', 'Value 1: 1 = zoom out, 0 = zoom in'],
		['[TRA] Play Scream Animation', ''],
		['[IV] Flash Virus', 'Value 1: message to flash (set blank for random)\nValue 2: transition state (not yet implemented)'],
		['[IV][LFD] command prompt', "Value 1: command prompt command type\nValue 2: command data variable"],
		['[IV] switch Stage Memory', 'Value 1: stage name'],
		['[IV] Toggle Camera Effect Status', "Value 1: Toggle on or off, behaviour covered by modchart"],
		...
		['[ST] change static opacity', 'Value 1: opacity to set to [opacity1;opacity 2;flicker period;flicker num]\nValue 2: fade time'],
		['[ST] fade overlay black screen of Blake', 'Value 1: opacity\nValue 2: fade time'],
		['[FA] Toggle animated backgrounds', "Value 1: Toggle State"],
		['[FA] Toggle double strum keys', "Value 1: 1 = toggle on, 0 = toggle off"],
		['[FA] change black cover opacity', "Value 1: Upper opacity fade (opacity,fade time)\nValue 2: Lower opacity fade"]
	];
	...
	**/
	...
	var slideInputText:FlxUIInputText;
	...
	function addNoteUI():Void
	{
		...
		slideInputText = new FlxUIInputText(10, 185, 180, "0");
		tab_group_note.add(slideInputText);
		blockPressWhileTypingOn.push(slideInputText);
		...
		tab_group_note.add(new FlxText(10, 170, 0, 'Slide ID'));
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			curSelectedNote[4] = Std.parseInt(slideInputText.text);
		}
	...
	function updateNoteUI():Void
	{
		if (curSelectedNote != null) {
			...
			slideInputText.text = '' + curSelectedNote[4];
		}
	}
	...
	//	undos.push(newsong);
		...
		var noteSus = 0;
		var slideTo = (Std.parseInt(slideInputText.text) == null ? 0 : Std.parseInt(slideInputText.text));
		var daAlt = false;
		...
		if(noteData > -1) {
			_song.notes[curSection].sectionNotes.push([noteStrum, noteData, noteSus, noteTypeIntMap.get(daType), slideTo]);
		...
		{
			_song.notes[curSection].sectionNotes.push([noteStrum, (noteData + 4) % 8, noteSus, noteTypeIntMap.get(daType), slideTo]);
		}
